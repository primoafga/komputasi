        ...
        <form class="form-horizontal" id="EditForm">
          ...
          <div class="modal-body">
            ...
            <label for="inputText" class="col-sm-2 col-form-label">GURU</label>
                <div class="col-sm-10">
                  <select name="GURU" class="form-control select2" id="eGURU" style="width: 100%;" >
                      ...
                      <option value="{{ $d->ID }}" >{{ $d->NAMA }} -> {{$d->MAPEL}}</option>
                      @endforeach
                  </select>
            ...
            <label for="inputText" class="col-sm-2 col-form-label">KELAS</label>
              <div class="col-sm-10">
                <select name="KELAS" class="form-control select2" id="eKELAS" style="width: 100%;" >
                    @foreach ($daftar_kelas as $d)
                    <option value="{{ $d->ID_KELAS }}" >{{ $d->NAMA }}</option>
                    @endforeach
                </select>
              </div>
            </div>
...
@section('script_e')
  <script>
    ...
    $('.EditBtn').on('click',function() {
      $cID = $(this).attr("value");
      if ($cID == "new") {
        $("#tJudul").text("Add Data KBM");
        $("#eGURU").val("");
        $("#eKELAS").val("");
        }
      else {
        $("#tJudul").text("Edit Data KBM");
        $.get("Det_Kbm/"+$(this).attr("value"))
          .done(function(data){
            $("#eGURU").val(data[0].ID_GURU);
            $("#eKELAS").val(data[0].ID_KELAS);
            $cID = data[0].ID;
          });
        }
