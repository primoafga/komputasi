        ...
        <form class="form-horizontal" id="EditForm">
          ...
          <div class="modal-body">
            <div class="form-group row">
              <label for="inputText" class="col-sm-2 col-form-label">NIG</label>
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <input type="text" name="NIG" id="eNIG"  class="form-control" placeholder="NIG" required>
                </div>
              </div>
              ...
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <input type="text" name="ALAMAT" id="eALAMAT"  class="form-control" placeholder="Alamat" required>
                </div>
              </div>
              <label for="inputText" class="col-sm-2 col-form-label">TTL</label>
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <input type="text" name="TTL" id="eTTL"  class="form-control" placeholder="TTL" required>
                </div>
              </div>
              <label for="inputText" class="col-sm-2 col-form-label">MENGAJAR</label>
              <div class="col-sm-10">
                <select name="MENGAJAR" class="form-control select2" id="eMENGAJAR" style="width: 100%;" >
                    @foreach ($daftar_mapel as $d)
                    <option value="{{ $d->ID_MAPEL }}" >{{ $d->NAMA }}</option>
                    @endforeach
                </select>
              </div>
            </div>
  ...
  <script>
    ...
    $('.EditBtn').on('click',function() {
      ...
      if ($cID == "new") {
        ...
        $("#tJudul").text("Edit Data Guru");
        $.get("Det_Gur/"+$(this).attr("value"))
          .done(function(data){
            $("#eNIG").val(data[0].NIG);
            $("#eNAMA").val(data[0].NAMA);
            $("#eALAMAT").val(data[0].ALAMAT);
            $("#eTTL").val(data[0].TTL);
            $("#eMENGAJAR").val(data[0].MENGAJAR);
            $cID = data[0].ID;
          });
        }
